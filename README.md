...
- [ ] **Measure latency in health checks** - `node.latency` is never updated (always 0), so the `(tier, latency)` sort in the routing table does nothing. Time the `getHealth` request in `get_health` and store the result in the atomic.
...
- [ ] **Remove or wire up `is_live`** - the `RpcNode::is_live` field is dead code; the routing table already serves that role.
